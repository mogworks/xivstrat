...
<template>
  <Dialog v-if="logs && logs.length > 0" v-model:open="isOpen">
    <DialogTrigger>
      <button
        class="flex w-max cursor-pointer items-center justify-between rounded-lg border border-zinc-400/80 bg-transparent p-2 px-4 align-middle shadow-xs transition-all hover:border-blue-400 hover:opacity-80"
        @click="handleOpen"
      >
