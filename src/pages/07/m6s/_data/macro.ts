/* eslint-disable no-irregular-whitespace */

// TODO: 需要正确性校验
export const macro1 = `/p ■ 基本分散 (北基准、TBOSS脚下)
/p 　      D3  D4
/p      H1　☆ 　H2
/p      　 D1  D2 
/p ■ 击飞分摊（2:2:2:2）
/p 　 　 　     BOSS
/p    H1D1　MTD4　H2D2
/p 　　　　   STD3
/p ■ 44分摊：←MT组 ST组→
/p ■ 沙漠阶段
/p ▼ 仙人掌(A点躲避) 　 ▼ 流沙双奶分散(危险区基准)
/p 　　　6人 　　　 　｜　H1　   6人
/p 　D　 　　　T　　  ｜　危险　 H2
/p ■ 炸弹(流沙北)
/p H1/D3 MT/D1 ST/D2 H2/D4`

export const macro2 = `/p ■小怪阶段（击杀顺序：马＞猫＞鱼＞松鼠＞羊）
/p wave1：MT松鼠　ST羊  近战羊→松鼠　远程羊→猫　远程奶妈羊脚下集合
/p wave2：D3引导D鱼　H2引导B鱼　北侧放水圈　MT拉松鼠到东→西鱼　其他DPS群攻
/p wave3：MT松鼠跟着马移动(眩晕：MT→D1→D2)　ST羊B点待机　点名奶妈南东
/p wave4：MT松鼠C　ST羊A(眩晕马：ST→D1→D2)　
/p D3拉D鱼　D4拉B鱼 北侧放水圈　MT拉松鼠到东鱼 ST拉羊到西鱼`

export const macro3 = `/p ■ 山川　　　　　　　    ■ 雷云
/p 火：外側MTH1D1D2　　 　 　 雷云为北
/p 　　内側STH2D3D4　　TH←左　人群　右→DPS
/p 雷：D3              D4
/p         D1    MT    ST
/p         H1    D2    H2
/p ■ 踩塔　　　　　 　       ■ 第二次塔
/p H2D4　北　H1D3　 　4・2・2塔：近战4人基准顺逆时针
/p ※※※※※※※※※※ 　面向场中左←【D1MTSTD2】【H1D3】【H2D4】→右
/p 　　 MT　　ST　　　 　 　　　　▼ 南8塔
/p 　　 D1　　D2　　　　　　　　H1　MT　H2
/p 分摊：D诱导　　　　　  　D3　D1　 ST　 D2 　D4`
